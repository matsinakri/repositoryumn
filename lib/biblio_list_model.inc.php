...
abstract class biblio_list_model
{
  ...
  protected $searchable_fields = array('title', 'author', 'subject', 'isbn',
	   'gmd', 'colltype', 'class', 'itemtype','callnumber', 'notes',
  ...
  public function JSONresult() {
    ...
    $data['resultInfo']['ResultNum'] = $this->num_rows;
    $data['resultInfo']['ResultPage'] = $this->current_page;
    $data['resultInfo']['ResultShowed'] = $this->num2show;
    ...
    while ($_biblio_d = $this->resultset->fetch_assoc()) {
      ...
      $document['title'] = trim($_title_main);
      ...
      while ($_auth_d = $_biblio_authors_q->fetch_assoc()) {
        ...
        $name['authors']['author_name'] = htmlspecialchars(trim($_auth_d['author_name']));
        $name['authors']['authority_type'] = $sysconf['authority_type'][$_auth_d['authority_type']];
      ...
      // ISBN
      $document['ISBN'] = str_replace(array('-', ' '), '', $_biblio_d['isbn_issn']);
     
      // imprint/publication data
      $document['publisher_name'] = $_biblio_d['publisher'];
      $document['publisher_year'] = $_biblio_d['publish_year'];

      // doc images
      $_image = '';
      if (!empty($_biblio_d['image'])) {
        $_image = urlencode($_biblio_d['image']);
        $document['image'] = $_image;
      }
    ...
    $_json_output = $data;
    return json_encode($_json_output, JSON_PRETTY_PRINT);
  }
