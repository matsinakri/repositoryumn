...
class biblio_list extends biblio_list_model
{
  protected $searchable_fields = array(
    ...
    'publisher', 'gmd', 'notes', 'colltype', 'publishyear', 'itemtype',
    'location', 'itemcode', 'callnumber', 'itemcallnumber', 'notes',
    'examiner', 'supervisor', 'contributor'
  );
  protected $field_join_type = array('title' => 'OR', 'author' => 'OR', 'examiner' => 'OR', 'supervisor' => 'OR', 'contributor' => 'OR', 'subject' => 'OR');
  ...
  public function setSQLcriteria($str_criteria)
  {
    ...
    foreach ($_queries as $_num => $_query) {
      ...
      if ($_field == 'title') {
      ...
      } else if ($_field == 'supervisor') {
        if ($_b == '-') {
          $_sql_criteria .= " biblio.biblio_id NOT IN(SELECT bs.biblio_id FROM biblio_supervisor AS bs"
            . " LEFT JOIN mst_supervisor AS s ON bs.supervisor_id=s.supervisor_id"
            . " WHERE supervisor_name LIKE '%$_q%')";
        } else {
          $_sql_criteria .= " biblio.biblio_id IN(SELECT bs.biblio_id FROM biblio_supervisor AS bs"
            . " LEFT JOIN mst_supervisor AS s ON bs.supervisor_id=s.supervisor_id"
            . " WHERE supervisor_name LIKE '%$_q%')";
        }
      } else if ($_field == 'examiner') {
        if ($_b == '-') {
          $_sql_criteria .= " biblio.biblio_id NOT IN(SELECT bs.biblio_id FROM biblio_examiner AS be"
            . " LEFT JOIN mst_examiner AS e ON be.examiner_id=e.examiner_id"
            . " WHERE examiner_name LIKE '%$_q%')";
        } else {
          $_sql_criteria .= " biblio.biblio_id IN(SELECT be.biblio_id FROM biblio_examiner AS be"
            . " LEFT JOIN mst_examiner AS e ON be.examiner_id=e.examiner_id"
            . " WHERE examiner_name LIKE '%$_q%')";
        }
      } else if ($_field == 'contributor') {
        if ($_b == '-') {
          $_sql_criteria .= " biblio.biblio_id NOT IN(SELECT bc.biblio_id FROM biblio_contributor AS bc"
            . " LEFT JOIN mst_contributor AS c ON bc.contributor_id=c.contributor_id"
            . " WHERE contributor_name LIKE '%$_q%')";
        } else {
          $_sql_criteria .= " biblio.biblio_id IN(SELECT bc.biblio_id FROM biblio_contributor AS bc"
            . " LEFT JOIN mst_contributor AS c ON bc.contributor_id=c.contributor_id"
            . " WHERE contributor_name LIKE '%$_q%')";
        }
        ...
        switch ($_field) {
          ...
          case 'itemtype':
            if (!$this->disable_item_data) {
              $_subquery = 'SELECT item_type_id FROM mst_item_type WHERE item_type_name=\'' . $_q . '\'';
              if ($_b == '-') {
                $_sql_criteria .= " biblio.item_type_id NOT IN ($_subquery)";
              } else {
                $_sql_criteria .= " biblio.item_type_id IN ($_subquery)";
              }
            }
            break;
          case 'notes':
