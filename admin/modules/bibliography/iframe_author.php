...
if ($biblioID) {
  ...
  if (isset($sysconf['plugin']['orcid']) AND $sysconf['plugin']['orcid'] !== false) {
      $biblio_author_q = $dbs->query("SELECT ba.*, a.author_name, a.author_year, a.authority_type, a.orcid_id FROM biblio_author AS ba
      LEFT JOIN mst_author AS a ON ba.author_id=a.author_id
      WHERE ba.biblio_id=$biblioID ORDER BY level ASC");          
  }
  $row = 1;
  while ($biblio_author_d = $biblio_author_q->fetch_assoc()) {
    ...
    //ORCID PLUGIN V1.0
    if (isset($sysconf['plugin']['orcid']) AND $sysconf['plugin']['orcid'] !== false) {
      $author_orcid = $biblio_author_d['orcid_id'];
    }

    $table->appendTableRow(array($remove_link, $author,(isset($author_orcid) ? $author_orcid : ""), $author_year, $authority_type, $sysconf['authority_level'][$biblio_author_d['level']]));
    ...
    $table->setCellAttr($row, 5, 'valign="top" class="'.$row_class.'" style="width: 20%;"');
    $row++;
  }
  ...
  if ($_SESSION['biblioAuthor']) {
    ...
    foreach ($_SESSION['biblioAuthor'] as $biblio_session) {
      ...
      if ($biblio_session) {
        $author_q = $dbs->query("SELECT author_name, author_year, authority_type, orcid_id FROM mst_author
        ...
        //ORCID PLUGIN V1.0
        if (isset($sysconf['plugin']['orcid']) AND $sysconf['plugin']['orcid'] !== false) {
          $author_orcid = $author_d[3];          
        }
      }

      $table->appendTableRow(array($remove_link, $author, (isset($author_orcid) ? $author_orcid : ""), $author_year, $authority_type, $sysconf['authority_level'][$biblio_session[1]]));
      $table->setCellAttr($row, 0, 'valign="top" class="'.$row_class.'" style="font-weight: bold; background-color: #ffc455; width: 10%;"');
      ...
      $table->setCellAttr($row, 5, 'valign="top" class="'.$row_class.'" style="background-color: #ffc466; width: 30%;"');
      $row++;
    }
