...
if (isset($sysconf['plugin']['orcid']) AND $sysconf['plugin']['orcid'] !== false) {
  include LIB.'orcid/orcid-service.inc.php';
}
...
// utility function to check author Orcid ID
function checkOrcidID($author_id, $str_author_type = 'p')
{
  global $dbs;
  $_q = $dbs->query('SELECT author_name,orcid_id FROM mst_author WHERE author_id=\''.$author_id.'\' AND authority_type=\''.$str_author_type.'\'');
  if ($_q->num_rows > 0) {
    $_d = $_q->fetch_row();
    // return the author ID
    return $_d;
...
function getOrcidID($author_name)
{ 
  $orcid = new OrcidService;
  return $orcid->getOrcidID($author_name);
}

// Setiadi 2 Author Type
$sysconf['authority_type'] = array('p' => 'Nama Orang');
$sysconf['authority_level'] = array(1 => 'Penulis');
...
if (isset($_POST['save']) AND (isset($_POST['authorID']) OR trim($_POST['search_str']))) {
  ...
  if (isset($_POST['biblioID']) AND !empty($_POST['biblioID'])) {
      ...
      if (isset($sysconf['plugin']['orcid']) AND $sysconf['plugin']['orcid'] !== false) {
        // check orcid id
        $orcid = checkOrcidID($data['author_id'], $_POST['type']);
        if ($orcid !== false) {
            if (empty($orcid[1])) {
              $orcid_id = getOrcidID($orcid[0]);
              if (!empty($orcid_id)) {
                  $update_data['orcid_id'] = $orcid_id;
                  $sql_op->update('mst_author', $update_data, "author_id = ".$data['author_id']);
              }
            }
        } 
      }
      ...
      if (isset($sysconf['plugin']['orcid']) AND $sysconf['plugin']['orcid'] !== false) {
        // check orcid id
        $orcid = checkOrcidID($author_id, $_POST['type']);
        if ($orcid !== false) {
            if (empty($orcid[1])) {
              $orcid_id = getOrcidID($orcid[0]);
              if (!empty($orcid_id)) {
                  $update_data['orcid_id'] = $orcid_id;
                  $sql_op->update('mst_author', $update_data, "author_id = ".$author_id);
              }
            }
        } 
      }
