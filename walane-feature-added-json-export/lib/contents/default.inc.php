...
if (isset($_GET['search']) && !empty($_GET['search'])) {
  ...
  if ($keywords && !preg_match('@[a-z0-9_.]+=[^=]+\s+@i', $keywords.' ')) {
    $criteria = 'title='.$keywords.' OR author='.$keywords.' OR supervisor='.$keywords.'  OR examiner='.$keywords.' OR contributor='.$keywords.' OR subject='.$keywords;
    $biblio_list->setSQLcriteria($criteria);
  } else {
    $biblio_list->setSQLcriteria($keywords);
  }
  // advanced search
  $is_adv = isset($_GET['title']) || isset($_GET['author']) || isset($_GET['supervisor']) || isset($_GET['contributor']) 
    || isset($_GET['examiner']) || isset($_GET['isbn'])
    || isset($_GET['subject']) || isset($_GET['location'])
    || isset($_GET['gmd']) || isset($_GET['colltype']) || isset($_GET['itemtype']) ||isset($_GET['publisher']) || isset($_GET['callnumber']);
  if ($is_adv) {
    $title = '';
    if (isset($_GET['title'])) {
    ...
    if (isset($_GET['supervisor'])) {
      $supervisor = trim(strip_tags(urldecode($_GET['supervisor'])));
    }
    $contributor = '';
    if (isset($_GET['contributor'])) {
      $contributor = trim(strip_tags(urldecode($_GET['contributor'])));
    } 
    $examiner = '';
    if (isset($_GET['examiner'])) {
      $examiner = trim(strip_tags(urldecode($_GET['examiner'])));
    }
    $subject = '';
    ...
    if (isset($_GET['itemtype'])) {
      $itemtype = trim(strip_tags(urldecode($_GET['itemtype'])));
    }
    $location = '';
    ...
    if ($title || $author || $supervisor || $examiner || $contributor || $subject || $isbn || $gmd || $colltype || $location || $publisher || $callnumber) {
      $criteria = '';
      if ($title) { $criteria .= ' title='.$title; }
      if ($author) { $criteria .= ' author='.$author; }
      if ($supervisor) { $criteria .= ' supervisor='.$supervisor; }
      if ($examiner) { $criteria .= ' examiner='.$examiner; }
      if ($contributor) { $criteria .= ' contributor='.$contributor; }
      if ($subject) { $criteria .= ' subject='.$subject; }
      if ($isbn) { $criteria .= ' isbn='.$isbn; }
      if ($gmd) { $criteria .= ' gmd="'.$gmd.'"'; }
      if ($colltype) { $criteria .= ' colltype="'.$colltype.'"'; }
      if ($itemtype) { $criteria .= ' itemtype="'.$itemtype.'"'; }
  ...
  if ($is_adv) {
    ...
    if ($examiner) { $search_result_info .= 'Examiner : <strong><cite>'.$examiner.'</cite></strong>, '; }
    ...
    if ($itemtype) { $search_result_info .= 'Item Type : <strong><cite>'.$itemtype.'</cite></strong>, '; }
  ...
  if (!isset($_GET['resultXML']) && !isset($_GET['resultJSON'])) {
    // show the list
  ...
  if (isset($biblio_list)) {
    if (isset($sysconf['enable_xml_result']) && $sysconf['enable_xml_result']) {
    ...
    if (isset($sysconf['enable_json_result']) && $sysconf['enable_json_result']) {
        $search_result_info .= '<div><a href="index.php?resultJSON=true&'.$_SERVER['QUERY_STRING'].'" class="xmlResultLink" target="_blank" title="View Result in JSON Format" style="clear: both;">JSON Result</a></div>';
    }
  }
}
...
// check if we are on xml resultset mode
if (isset($_GET['resultJSON']) && $sysconf['enable_json_result']) {
  // get document list but don't output the result
  $biblio_list->getDocumentList(false);
  if ($biblio_list->num_rows > 0) {
      header('Content-Type: text/json');
      echo $biblio_list->JSONresult();
  }
  exit();
}